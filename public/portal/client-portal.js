(() => {
  ...
  function ideaCard(idea) {
    const recommended = idea.number === "HYBRID" ? " recommended" : "";
  ...
  function renderHome() {
    ...
    const hero = `<section class="hero-panel"><p class="eyebrow">Relationship mission / Third i</p><h2>Turn bkWatch's operational authority into clear, approved product proof.</h2><p>Third i can support the relationship from sourced strategy and acquisition systems through film, web, analytics, communications, and workflow automation—while preserving bkWatch's credibility, privacy boundaries, and human approval gates.</p><div class="hero-flags"><span><strong>Current focus</strong> Build selected demo</span><span><strong>Client system</strong> Rules-based automation baseline</span><span><strong>Next gate</strong> Demo review after build</span></div></section>`;
