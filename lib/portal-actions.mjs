...
export async function listActions(store, tenant) {
  /* Reconciliation: scan for comments that have no corresponding action and
     create one. This catches comments created before the action system existed,
     or any that were missed due to errors. Runs on every GET so the OS always
     reflects the full comment set. */
  await syncCommentActions(store, tenant);
...
/** Find comments without a linked action and create one for each. Also hides
    actions whose comment was soft-deleted. Idempotent — safe to run on every
    GET request. */
async function syncCommentActions(store, tenant) {
  const commentKeys = await store.list(key(tenant, "comments") + "/");
  const actionKeys = await store.list(key(tenant, "actions") + "/");
  /* Build a set of subjectIds that already have actions. */
  const existingSubjectIds = new Set();
  for (const ak of actionKeys) {
    const action = await store.get(ak);
    if (action && action.subjectId) existingSubjectIds.add(action.subjectId);
  }
  /* Create actions for comments that don't have one yet. */
  for (const ck of commentKeys) {
    const comment = await store.get(ck);
    if (!comment || comment.deleted) continue;
    if (!existingSubjectIds.has(comment.id)) {
      await createFromComment(store, tenant, comment, null);
    }
  }
  /* Hide actions whose comment was soft-deleted. */
  for (const ak of actionKeys) {
    const action = await store.get(ak);
    if (!action || !action.subjectId || action.status === "hidden") continue;
    const comment = await store.get(key(tenant, "comments", action.subjectId));
...
export async function patchAction(store, tenant, id, patch) {
  ...
  /* Swap priority with an adjacent action — moves one space up/down
     instead of jumping to the top/bottom of the list. */
  if (patch.op === "swap" && patch.swapId) {
    const other = await getAction(store, tenant, patch.swapId);
    if (!other) return { ok: false, status: 404, error: "swap_target_not_found" };
    const tmp = action.priority || 0;
    action.priority = other.priority || 0;
    other.priority = tmp;
    action.updatedAt = nowIso(); other.updatedAt = nowIso();
    await store.set(key(tenant, "actions", other.id), other);
