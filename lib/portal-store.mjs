...
export function portalStoreVersion(env = process.env) {
  if (env.PORTAL_DATA_STORE_VERSION) return env.PORTAL_DATA_STORE_VERSION;
  const context = env.CONTEXT || "local";
  if (context === "deploy-preview" || context === "branch-deploy") return "portal-preview-v1";
  if (context === "production") return "portal-live-v1";
  return "portal-local-v1";
}
export function assertPortalStoreIsolation(env = process.env) {
  const context = env.CONTEXT || "local";
  const version = portalStoreVersion(env);
  if ((context === "deploy-preview" || context === "branch-deploy") && version === "portal-live-v1") {
    throw new Error("preview_context_cannot_use_live_portal_store");
  }
  if (context === "production" && version === "portal-preview-v1") {
    throw new Error("production_context_cannot_use_preview_portal_store");
  }
  return version;
}
const STORE_NAME = portalStoreVersion();
